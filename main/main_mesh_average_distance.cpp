...
int main()
{
	...
	double Re=0.02;
	double LZ=nz;	
	double gx=0.0, gy=0.0, gz=0.0, uw_top=1.0/300.0, uw_bottom=-1.0/300.0, den0=1.0, vis0=1.0/6;
	...
	const int N_par=1;
	particle par[N_par];
	xave=f.LX/2.0, yave=f.LY/2.0, zave=f.LZ/2.0, R=4.0;
	par[0].mesh_read();
	...
	//xave=f.LX/2.0-2*R-0.5, yave=f.LY/2.0, zave=6, R=3.0;
	//par[1].mesh_read();
	...
	for (i=0;i<=N_par-1;i++)
	{
		...
		par[i].F_Fb_setnum();
		//弹性常数
		par[i].ks=2.17/1000; par[i].k_alpha=par[i].ks; par[i].kb=0; par[i].ka=0; 
		par[i].kv=0; 
	...
	//ofstream foutD;
	//foutD.open("./result/result_particle_one_D.txt",ios::out);
	
	par[0].mesh_average_node_distance_cal();	
	...
	//	cout<<"particle "<<i<<" D= "<<par[i].D<<endl;
	//}
	return 0;
}

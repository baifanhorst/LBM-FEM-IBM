...
{
	//流场 分配空间
	//把R, dR, Re, Ca这四个量放在最前面，便于修改
	double R=6; //初始细胞半径
	double dR=0; //等效半径修正
	double Re=0.02;
	double Ca=0.1;
	double ks_ratio=1; //等效ks比例
	int nx=20*int(R), ny=5*int(R), nz=20*int(R);
	flow f(nx,ny,nz);
	//流场 C流初始化
	
	double LZ=nz;	
	double gx=0.0, gy=0.0, gz=0.0, uw_top=Re*LZ/12/R/R, uw_bottom=-uw_top, den0=1.0, vis0=1.0/6;
	...
	xave=f.LX/2.0, yave=f.LY/2.0, zave=f.LZ/2.0+0.5*(R+dR);
	par[0].mesh_read();
	...
	par[1].mesh_read();
	xave=f.LX/2.0+2.0*(R+dR), yave=f.LY/2.0, zave=f.LZ/2.0-0.5*(R+dR);
	...
	for (i=0;i<=N_par-1;i++)
	{
		...
		par[i].F_Fb_setnum();
		//弹性常数
		par[i].ks=Re/(36*R*Ca)*ks_ratio; par[i].k_alpha=par[i].ks; par[i].kb=0; par[i].ka=0; 
	...
	//foutdz.open("./result/result_particle_dz",ios::out);
	...
	for (i=0;i<=N_par-1;i++)
	{
		...
		cout<<"particle center "<<par[i].pos_center[0]<<' '<<par[i].pos_center[1]<<' '<<par[i].pos_center[2]<<endl;
	}
	...
	for (step=1;step<=tmax;step++)
	{
		...
		if (t%100==0)
		{
			...
			foutdz<<par[0].pos_center[0]-par[1].pos_center[0]<<' '<<par[0].pos_center[2]-par[1].pos_center[2]<<endl;
